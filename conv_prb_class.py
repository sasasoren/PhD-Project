...
mon_freq = 100
test_num = 5
Save_adress = 'Images/conv_prb_class'
...
with tf.Session() as sess:
    ...
    cls_3 = class_mat.copy()
    cls_3[np.where((x_pr>.1) & (x_pr<.9))] = 2
...
x_train, y_train, x_test, y_test, x_, y_, label_x = win_ftset_and_label(img, img_cut, cls_3,
                                                                        test_size=.1,
                                                                        win_size=window_size,
...
def cn_net2(x_h):
    ...
    # Output layer, class prediction
    out = tf.layers.dense(fc1, num_classes+1)
    return out


x_pred = tf.nn.sigmoid(cn_net2(x_h))

cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=cn_net2(x_h), labels=y_all))
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)
...
confusion = tf.confusion_matrix(labels=y_idx, predictions=pred_idx, num_classes=num_classes+1)
